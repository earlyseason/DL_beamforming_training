...
from models import Beam_Estimator_1D, Beam_Estimator_FC, count_parameters
...
def main():
    ...
    print("  消融实验总结")
    print("="*60)
    print(f"{'配置':<30} {'Top-1':<10} {'Top-3':<10} {'SNR Loss':<10}")
    print("-" * 60)

    for name, res in results.items():
        print(f"{name:<30} {res['top1_acc']*100:>6.2f}%   "
    ...
    print(f"  最佳配置 vs Baseline:     {(fc_kl_top1 - baseline_top1)*100:+.2f}%")
